...
begin
 ...
 case (status) of
  no_nuz, nuzing: oblr.dk_blik := false;
  blk_in_nuz: oblr.dk_blik := true;
 end;
end;
...
begin
 ...
 if (BridgeClient.authStatus = TuLIAuthStatus.yes) then Result := Result + ',MAUS loko';
end;
...
begin
 ...
 case (Self.myORs[Sender].tech_rights) of
  TORControlRights.read      : rs := 'ke čtení';
 ...
 Application.MessageBox(PChar('Oblast řízení : ' + Self.myORs[Sender].Name + #13#10 +
                              'ID : ' + Self.myORs[Sender].id + #13#10 +
                              'Přihlášen : ' + Self.myORs[Sender].username + #13#10 +
                              'Lichý směr : ' + lichy + #13#10 +
                              'Oprávnění : ' + rs),
...
begin
 ...
 else if (sp_menu = 'LOKO') then begin
...
begin
 ...
 else PanelTCPClient.PanelDkMenuClick(Self.myORs[obl_r].id, item); // no special menu -> send click to server
...
begin
 ...
 else if (item = 'PŘEDAT loko') then F_HV_Move.Open(Self.myORs[obl_r].id, Self.myORs[obl_r].HVs)
...
begin
 ...
 for i := 1 to Self.myORs.Count-1 do
   if (res = '-') then
     ...
     if (Self.myORs[i].username <> res) then Exit('více uživatelů');
