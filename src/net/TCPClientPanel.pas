...
begin
  ...
  // send handshake
  Self.SendLn('-;HELLO;' + Self._PROTOCOL_VERSION_CLIENT + ';' + Self._PROTOCOL_APP_NAME);
end;
...
begin
  ...
  begin
    case (parsed[2][1]) of
      '0':
        Relief.ORDKClickServer(parsed[0], false);
      '1':
        Relief.ORDKClickServer(parsed[0], true);
    end; // case
...
begin
  case (typ) of
    _STITEK:
      Self.SendLn('-;STIT;{' + stitvyl + '}');
    _VYLUKA:
      Self.SendLn('-;VYL;{' + stitvyl + '}');
  end;
end;
...
begin
  ...
  begin
    ...
    try
      case (list2[1][1]) of
        '0':
          Relief.OROsvChange(oblr, list2[0], false);
        '1':
          Relief.OROsvChange(oblr, list2[0], true);
      end;
    except
