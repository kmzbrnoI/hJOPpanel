...
begin
  ...
  if ((Assigned(F_Auth)) and ((F_Auth.Showing) or (F_Auth.listening))) then
    F_Auth.UpdateULIcheckbox();
  ...
  if (Assigned(Self.OnAuthStatushanged)) then
    Self.OnAuthStatushanged(Self);
end;
...
begin
  ...
  begin
    if (parsed[1] = 'no') then
      Self.fAuthStatus := TuLIAuthStatus.no
    else if (parsed[1] = 'yes') then
      Self.fAuthStatus := TuLIAuthStatus.yes
    else if (parsed[1] = 'cannot') then
      Self.fAuthStatus := TuLIAuthStatus.cannot;
    ...
    if ((Self.authStatus = TuLIAuthStatus.no) and (Self.toLogin.server <> '') and (Self.toLogin.username <> '') and
      (Self.toLogin.password <> '')) then
      Self.Auth();

  end else if (parsed[0] = 'LOKO') then
  begin
    if (parsed[1] = 'ok') then
      asm nop;
      end
      // TODO
    else if (parsed[1] = 'err') then
    begin
...
begin
  Self.SendLn('LOGIN;{' + Self.toLogin.server + '};' + IntToStr(Self.toLogin.port) + ';{' + Self.toLogin.username +
