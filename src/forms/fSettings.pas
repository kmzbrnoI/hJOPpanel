...
begin
  ...
  begin
    ...
    if (Self.guestPasswdChanged) then
      GlobConfig.data.guest.password := GenerateHash(Self.E_Guest_Password.Text);
  end else begin
...
begin
  case (Sender as TButton).Tag of
    1:
      Self.OD_Snd.InitialDir := ExtractFileDir(ExpandFileName(Self.E_Snd_Trat.Text));
    2:
      Self.OD_Snd.InitialDir := ExtractFileDir(ExpandFileName(Self.E_Snd_Error.Text));
    3:
      Self.OD_Snd.InitialDir := ExtractFileDir(ExpandFileName(Self.E_Snd_PS.Text));
    4:
      Self.OD_Snd.InitialDir := ExtractFileDir(ExpandFileName(Self.E_Snd_Pretizeni.Text));
    5:
      Self.OD_Snd.InitialDir := ExtractFileDir(ExpandFileName(Self.E_Snd_Zprava.Text));
    6:
      Self.OD_Snd.InitialDir := ExtractFileDir(ExpandFileName(Self.E_Snd_Privolavacka.Text));
    7:
      Self.OD_Snd.InitialDir := ExtractFileDir(ExpandFileName(Self.E_Snd_Timeout.Text));
    8:
      Self.OD_Snd.InitialDir := ExtractFileDir(ExpandFileName(Self.E_Snd_StaveniVyzva.Text));
    9:
      Self.OD_Snd.InitialDir := ExtractFileDir(ExpandFileName(Self.E_Snd_NeniJC.Text));
  end;
  ...
  begin
    ...
    case (Sender as TButton).Tag of
      1:
        Self.E_Snd_Trat.Text := fn;
      2:
        Self.E_Snd_Error.Text := fn;
      3:
        Self.E_Snd_PS.Text := fn;
      4:
        Self.E_Snd_Pretizeni.Text := fn;
      5:
        Self.E_Snd_Zprava.Text := fn;
      6:
        Self.E_Snd_Privolavacka.Text := fn;
      7:
        Self.E_Snd_Timeout.Text := fn;
      8:
        Self.E_Snd_StaveniVyzva.Text := fn;
      9:
        Self.E_Snd_NeniJC.Text := fn;
    end;
  end;
end;
...
begin
  if (Self.LB_Symbols.ItemIndex > -1) then
    Self.B_ApplyClick(B_Apply);
end;
...
begin
  ...
  if (Self.CHB_Forgot.Checked) then
    Self.CHB_Forgot.enabled := false;
