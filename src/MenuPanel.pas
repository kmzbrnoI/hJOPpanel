...
begin
 ...
 Canvas.Rectangle(
  SymbolSet._Symbol_Sirka,
  SymbolSet._Symbol_Vyska,
  SymbolSet._Symbol_Sirka*(Self._MENU_WIDTH+1),
  SymbolSet._Symbol_Vyska*(Self.Items.cnt+3));

 // ramecek
 Canvas.Pen.Color := clBlack;
 Canvas.Rectangle(
  Round(SymbolSet._Symbol_Sirka*1.5),
  Round(SymbolSet._Symbol_Vyska*1.5),
  ...
  begin
   ...
   if (Self.Items.data[i].disabled) then
    begin
     ...
     Pos.X := _PNL_MENU_ITEM_LEFT_OFFSET;
     foreground := clBlack;
     if (Self.Items.data[i].admin) then background := clTeal;
    end;
   end;
    ...
    begin
     if ((Self.Items.data[i].show_text = '-') or (Self.Items.data[i].disabled) or (Self.Items.data[i].header)) then
      Self.fselected := -1
       ...
       if (background = clTeal) then foreground := clYellow;
       background := $5555CC;
       Self.fselected := i;
       if (foreground = clRed) then foreground := clYellow;
     end;
    end;
   ...
   if (Self.Items.data[i].show_text = '-') then
    begin
     ...
     Canvas.LineTo(
      Round(((_PNL_MENU_ITEM_LEFT_OFFSET+_MENU_WIDTH-2)*SymbolSet._Symbol_Sirka)+(SymbolSet._Symbol_Sirka/2)),
      (Pos.Y * SymbolSet._Symbol_Vyska)+Round(SymbolSet._Symbol_Vyska/2)
     );
    end else
